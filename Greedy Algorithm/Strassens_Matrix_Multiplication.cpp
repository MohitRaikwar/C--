...
void Strassen_algorithm(vector<vector<int>> &A, vector<vector<int>> &B, int m, int n, int a, int b)
{
    ...
    cout << "\n\nMATRIX AFTER MULTIPLICATION (STRASSEN'S METHORD) :\n";
    display(C, m, b);
}

void MatrixMultiplication(vector<vector<int>> &A, vector<vector<int>> &B, int m, int n, int o)
{ // MatrixMultiplication() FUNCTION FOR (NAIVE MATRIX MULTIPLICATION)
    vector<vector<int>> C;
    int sum, i, j, k;
    for (i = 0; i < m; i++)
    {
    ...
    cout << "\n\nMATRIX AFTER MULTIPLICATION (NAIVE METHORD) :\n";
    display(C, m, o);
}
...
int main()
{
    ...
    int p1, p2, q1, q2, num, i, j, k;

    cout << "ENTER DIMENTION OF FIRST MATRIX :\n";
    cin >> p1 >> q1;
    cout << "\nENTER DIMENTION OF SECOND MATRIX :\n";
    cin >> p2 >> q2;

    if (q1 != p2) // CONDITION CHECK FOR MATRIX MULTIPLICATION
        cout << "\n\nOhh.....MATRIX MULTIPLICATION NOT POSSIBLE !......\n";
    else
    {
        cout << "\n\nENTER FIRST MATRIX :\n";
        for (i = 0; i < p1; i++)
    ...
    if (p1 == q1 && p2 == q2 && q1 == p1 && p1 % 2 == 0) // CONDITION CHECK FOR STRASSEN'S MATRIX MULTIPLICATION
        Strassen_algorithm(a, b, p1, p2, q1, q2);
    else if (q1 == p2) // CONDITION CHECK FOR NAIVE MATRIX MULTIPLICATION
        MatrixMultiplication(a, b, p1, q1, q2);
